    ...
    <div class="message-main">
      ...
      <div v-if="message.images?.length" class="generated-images">
        <a v-for="(image, index) in message.images" :key="index" :href="image.url || image" target="_blank" rel="noreferrer">
          <img :src="image.url || image" :alt="image.label || 'AI 生成的图片'" />
        </a>
      </div>
      <div class="bubble">
...
<style scoped>
.message { display:flex; gap:11px; width:min(100%, 900px); margin:0 auto 24px; }.message.user { flex-direction:row-reverse; }.avatar { flex:none; display:grid; place-items:center; width:34px; height:34px; border-radius:11px; color:var(--brand); background:var(--brand-soft); font-size:12px; font-weight:700; }.user .avatar { color:white; background:var(--brand); }.message-main { position:relative; min-width:0; max-width:min(86%, 760px); }.user .message-main { display:flex; flex-direction:column; align-items:flex-end; }.bubble { border:1px solid var(--line); border-radius:6px 16px 16px 16px; padding:14px 16px; color:var(--text); background:var(--surface); box-shadow:0 4px 14px rgba(21,48,38,.025); line-height:1.78; white-space:pre-wrap; overflow-wrap:anywhere; }.user .bubble { border-radius:16px 6px 16px 16px; color:white; background:var(--brand); box-shadow:0 6px 16px color-mix(in srgb, var(--brand) 16%, transparent); }.bubble p { display:grid; margin:0; border-radius:5px; transition:background .2s,color .2s; }.bubble p + p { margin-top:7px; }.stream-visible,.stream-sizer { grid-area:1 / 1; min-width:0; }.stream-visible { position:relative; z-index:1; }.stream-sizer { visibility:hidden; pointer-events:none; }.bubble p.speaking { margin-right:-4px; margin-left:-4px; padding:0 4px; color:var(--brand); background:var(--brand-soft); font-weight:600; }.user .bubble p.speaking { color:white; background:rgba(255,255,255,.18); }.cursor { display:inline-block; width:7px; height:1.2em; margin-left:4px; vertical-align:-.2em; background:var(--brand); animation:blink .8s infinite; }.message-actions { display:flex; gap:4px; min-height:23px; margin-top:4px; opacity:.4; }.message:hover .message-actions { opacity:1; }.message-actions button { width:26px; height:24px; border:0; border-radius:6px; color:var(--muted); background:transparent; }.message-actions button:hover { color:var(--brand); background:var(--surface-soft); }.thinking-detail,.agent-detail { margin:0 0 9px; border:1px solid var(--line); border-radius:10px; color:var(--muted); background:var(--surface-soft); font-size:13px; }.thinking-detail summary,.agent-detail summary { padding:10px 12px; cursor:pointer; color:var(--brand); font-weight:600; }.thinking-detail summary span { color:var(--muted); font-weight:400; }.thinking-detail pre { max-height:230px; overflow:auto; margin:0; padding:0 12px 12px; white-space:pre-wrap; overflow-wrap:anywhere; font:12px/1.6 ui-monospace,monospace; }.agent-step { padding:8px 12px; border-top:1px solid var(--line); }.message-status { position:absolute; z-index:4; top:0; left:8px; max-width:calc(100% - 16px); margin:0; overflow:hidden; transform:translateY(calc(-100% - 4px)); border:1px solid var(--line); border-radius:999px; padding:3px 8px; color:var(--muted); background:var(--surface); font-size:11px; line-height:1.3; text-overflow:ellipsis; white-space:nowrap; pointer-events:none; }.attachment-preview,.generated-images { display:flex; gap:8px; flex-wrap:wrap; margin:0 0 9px; }.attachment-preview img { width:140px; max-height:140px; object-fit:cover; border:1px solid var(--line); border-radius:10px; }.attachment-preview span { padding:7px 9px; border:1px solid var(--line); border-radius:7px; color:var(--muted); font-size:12px; }.generated-images img { display:block; max-width:min(100%, 420px); max-height:440px; border-radius:13px; object-fit:contain; background:var(--surface-soft); }.source-list { display:grid; gap:3px; margin-top:8px; }.source-list a { overflow:hidden; color:var(--brand); font-size:12px; text-overflow:ellipsis; white-space:nowrap; }@keyframes blink{50%{opacity:0}}
@media (max-width:720px) { .message { width:100%; margin-bottom:16px; }.avatar { width:28px; height:28px; font-size:10px; }.message-main { max-width:calc(100% - 38px); }.bubble { padding:11px 12px; line-height:1.65; font-size:14px; }.generated-images img { max-width:100%; }.message-actions { opacity:1; } }
</style>
